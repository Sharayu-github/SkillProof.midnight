/**
 * Deploy the SkillProof contract to a Midnight network (default: local devnet;
 ...
 *
 * Privacy note: only the derived issuer public key leaves this machine as a
 * constructor argument. The issuer's secret key and the initial empty profile
 * live in the wallet's private state and are never published.
 */
...
// Identifier under which the issuer's private state is stored in the level DB.
const PRIVATE_STATE_ID = 'verificationPrivateState';
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy SkillProof to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  // Faucet poll for public networks (same idiom as deploy in the demo).
  ...
  // The issuer's identity. The secret key is derived deterministically from the
  // per-network seed; only the derived public key is passed to the contract
  // constructor (it is public by design — it gates the issuer-only circuits).
  const issuerSecretKey = bytesFromHex(seed);
  const issuerKey = issuerKeyOf(issuerSecretKey);

  // The issuer's initial private state: profile + opening + empty certificate
  // list. This lives only in the level DB on this machine.
  const initialPrivateState: VerificationPrivateState = {
    secretKey: issuerSecretKey,
    profile: makeProfile({
      name: 'SkillProof Issuer',
      email: 'issuer@skillproof.local',
      college: 'SkillProof Platform',
      examScore: 0,
      cgpa: 0,
      skills: 'issuer',
    }),
